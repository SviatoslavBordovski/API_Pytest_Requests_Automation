...
class RequestsUtility(object):
    ...
    def post(self, endpoint, body_params=None, headers=None, expected_status_code=200):
        ...
        rs_api = requests.post(url=self.url, data=json.dumps(body_params), headers=headers, auth=self.auth)
    ...
    def get(self, endpoint, body_params=None, headers=None, expected_status_code=200):
        ...
        self.url = self.base_url + endpoint
        rs_api = requests.get(url=self.url, data=json.dumps(body_params), headers=headers, auth=self.auth)
        self.status_code = rs_api.status_code
        self.expected_status_code = expected_status_code
        self.rs_json = rs_api.json()
        self.assert_status_code()
